...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-surface flex selection:bg-gold/30 selection:text-navy transition-colors duration-500">
      {/* Sidebar - Desktop */}
      <aside className="w-72 bg-navy hidden lg:flex flex-col text-white relative z-20 shadow-2xl transition-colors duration-500">
        <div className="p-10">
          <Link to="/" className="flex items-center gap-3 group">
            <img src="/logo.png" alt="PayDish" className="h-8 w-auto" />
            <span className="text-xl font-black tracking-tighter text-white uppercase">PayDish</span>
          </Link>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all duration-300 font-bold text-sm ${
                activeTab === item.id 
                ? 'bg-white/10 text-gold shadow-sm shadow-black/20' 
            ...
            >
              ...
              {item.badge && (
                <span className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-black ${
                  activeTab === item.id ? 'bg-gold text-navy' : 'bg-white/10 text-white/40'
                }`}>
                  {item.badge}
                </span>
              )}
        ...
        <div className="p-8 mt-auto">
          <div className="bg-white/5 border border-white/10 p-5 rounded-3xl flex items-center gap-4">
            <div className="w-10 h-10 bg-gold rounded-xl flex items-center justify-center font-black text-navy shadow-lg text-xs">AK</div>
      ...
      <main className="flex-1 h-screen overflow-y-auto w-full relative z-10 bg-surface-2 transition-colors duration-500">
        ...
        <header className="sticky top-0 z-30 bg-surface-2/80 backdrop-blur-md px-10 py-6 flex items-center justify-between border-b border-border transition-colors duration-500">
          <div>
            <h1 className="text-2xl font-black text-navy tracking-tight">Bienvenue, Adjoua.</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="p-3 bg-surface rounded-xl border border-border text-ink-muted relative hover:text-gold transition-colors shadow-sm">
              <Bell size={20} />
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                   {[
                     ...
                     { label: 'Commandes actives', value: '12', icon: ShoppingBag, trend: null, color: 'text-gold' },
                     { label: 'Temps moyen', value: '14 min', icon: Clock, trend: '-2m', color: 'text-success' },
                     { label: 'Satisfaction', value: '4.9/5', icon: Star, trend: null, color: 'text-gold' }
                   ].map((stat, i) => (
                     <div key={i} className="bg-white p-6 rounded-[2rem] border border-border shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                           <div className="w-10 h-10 bg-surface-2 rounded-xl flex items-center justify-center text-navy">
                              <stat.icon size={20} />
                           </div>
                ...
                <div className="grid xl:grid-cols-3 gap-8">
                   <div className="xl:col-span-2 bg-white rounded-[2.5rem] border border-border p-10 shadow-sm relative overflow-hidden">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                         <div>
                            <h3 className="text-xl font-black text-navy">Analyse des Revenus</h3>
                            <p className="text-xs font-medium text-ink-muted mt-1">Comparaison des 7 derniers jours</p>
                         </div>
                         <div className="flex gap-2">
                            <button className="px-5 py-2 bg-navy text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Hebdomadaire</button>
                            <button className="px-5 py-2 bg-surface-2 text-ink-muted rounded-xl text-[10px] font-black uppercase tracking-widest hover:text-navy transition-colors">Mensuel</button>
                         </div>
                      </div>
                         ...
                         <div className="absolute inset-0 flex items-end justify-between gap-4">
                            {[40, 70, 45, 90, 65, 85, 55].map((h, i) => (
                               <div key={i} className="flex-1 group relative flex flex-col items-center">
                                  <div className="w-full bg-surface-2 rounded-2xl relative overflow-hidden transition-all duration-500 hover:bg-gold/10" style={{ height: `${h}%` }}>
                                     <div className="absolute bottom-0 left-0 right-0 bg-gold/30 rounded-2xl" style={{ height: `${h/2}%` }}></div>
                                     <div className="absolute top-0 left-0 right-0 bg-navy opacity-5 h-full transition-all group-hover:opacity-0"></div>
                                  </div>
                   ...
                   {/* Insight Card */}
                   <div className="bg-navy text-white rounded-[2.5rem] p-10 shadow-xl relative overflow-hidden flex flex-col justify-between">
                      <div className="relative z-10">
                         <div className="w-12 h-12 bg-gold/20 rounded-2xl flex items-center justify-center text-gold mb-8">
                            <Sparkles size={24} />
                         </div>
                         <h3 className="text-2xl font-black mb-4 leading-tight">Insight PayDish AI</h3>
                         <p className="text-white/60 text-sm font-medium leading-relaxed mb-10">
                            Le plat <span className="text-gold font-bold">"Atassi Royale"</span> performe 40% mieux le samedi. Pensez à augmenter votre stock de piment noir.
                         </p>
                      </div>
                      <button className="w-full bg-white text-navy py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest relative z-10 shadow-xl hover:bg-gold transition-colors">
                         Optimiser les stocks
                      </button>
                      <div className="absolute top-[-10%] right-[-10%] w-64 h-64 bg-gold/10 blur-[80px] rounded-full"></div>
                   </div>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-border shadow-sm overflow-hidden">
                   <div className="p-8 border-b border-border flex items-center justify-between">
                      <h3 className="text-xl font-black text-navy">Commandes Récentes</h3>
                      <button className="text-[10px] font-black text-gold uppercase tracking-widest border-b-2 border-gold/20 hover:border-gold transition-all">Tout l'historique</button>
                   </div>
                            ...
                            {MOCK_ORDERS.map((order, i) => (
                               <tr key={i} className="hover:bg-surface-2 transition-colors">
                                  <td className="px-8 py-6">
                                     <div className="text-sm font-bold text-navy">#PAY-{order.id.split('-')[1]}</div>
                                     <div className="text-[10px] text-ink-muted font-medium uppercase mt-1">Il y a 12 min</div>
                                  </td>
                                  <td className="px-8 py-6">
                                     <span className="px-3 py-1 bg-surface-2 rounded-lg text-xs font-black text-navy border border-border">Table {order.tableNumber}</span>
                                  </td>
                                  <td className="px-8 py-6 font-bold text-sm text-navy">{order.total.toLocaleString()} FCFA</td>
                                  <td className="px-8 py-6">
                                  ...
                                  <td className="px-8 py-6">
                                     <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                                        order.status === 'preparing' ? 'bg-gold/10 text-gold' : 'bg-success/10 text-success'
                                     }`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                   <div className="flex gap-4">
                      {restaurant.categories.map((cat, i) => (
                        <button key={i} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${i === 0 ? 'bg-navy text-white shadow-lg' : 'bg-white border border-border text-ink-muted hover:text-navy'}`}>
                           {cat.name}
                        </button>
                      ))}
                   </div>
                     ...
                     <div key={i} className="bg-white rounded-[2.5rem] border border-border overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 group">
                        <div className="h-56 relative overflow-hidden">
                           ...
                           <div className="absolute inset-0 bg-navy/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                              <button className="p-3 bg-white text-navy rounded-xl hover:bg-gold hover:text-white transition-colors shadow-xl"><Edit3 size={18}/></button>
                        ...
                        <div className="p-8">
                           <div className="flex justify-between items-start mb-2">
                              <h4 className="text-xl font-black text-navy">{item.name}</h4>
                              <span className="font-bold text-gold">{item.price.toLocaleString()} FCFA</span>
                           </div>
                           ...
                           <div className="pt-6 border-t border-border flex items-center justify-between">
                              ...
                              <button className="text-[10px] font-black text-navy uppercase tracking-widest hover:text-gold transition-colors">Modifier</button>
                           </div>
                        </div>
                     </div>
                   ))}
